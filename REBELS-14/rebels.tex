...
\CEU.
%
Organisms extends objects with an execution body composed of multiple lines of 
...
FRP and raising the historical dichotomy between functional and imperative 
...
%
The code chunks that follows compares the \code{par/and} and \code{par/or} 
...
and the event \code{move} to listen requests to move the unit position (line 
3).
%
...
terminated and being freed.

To protect from dangling pointers, \CEU enforces all pointer accesses across 
reactions to use the \code{watching} construct which supervises organism 
