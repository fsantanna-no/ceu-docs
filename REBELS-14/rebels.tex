...
\end{itemize}
%
In the code in the middle, the Blink class (lines 1-8) exposes the \code{port} 
and \code{dt} fields, which correspond to the LED port and blinking period to 
be configured for each instance.
...
The rightmost code is semantically equivalent to the one in the middle, with 
the organisms constructors and bodies expanded (lines 11-15 and 19-23) in a 
...
From the expansion, i

- survive reactions
- memory static
- relies on par/or
    - all finalization will also properly clean an organism going out of scope
