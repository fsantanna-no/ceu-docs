...
mathescape=true,
    ...
    emph={par,or,hor,do,end,loop,await,emit,input,event,call,with,command,%
          ...
          class, every, FOREVER, this, spawn, in, pool,
...
%
The lack of abstractions with dynamic lifetime makes difficult to deal with 
virtual resources such as graphical widgets, game objects, and network 
transactions, we need a form of abstraction
...
\section{Organisms: Reactive Abstractions}
\label{sec.ceu.orgs}
...
%
As commented in the introduction, objects are the standard way for abstraction 
for reactive ...
...
possible to keep the application responsive in the observer 
pattern~\cite{games.patterns}.
...
\CEU also supports dynamic instantiation of organisms through the \code{spawn} 
primitive.
...
Pools support iterators that return pointers to alive instances.
The example that follows iterates over all alive units and requests them to 
...
At any time
Note that programs do not require holding references
because the organisms are reactive

Given that organisms react independently to the environment, it is often not 
necessary to hold pointers returned by \code{spawn} invocations (as illustrated 
in line 6 of Figure~\ref{lst.pool}).
%
can be recovered through iterators

Pools also have the advantage
static
embedded systems
