...
%
The rigorous semantics of Esterel, which focus on static safety guarantees, is 
not suitable for other reactive application domains, such as GUIs, games, and 
distributed systems.
%
...
The code of Figure~\ref{lst.ctp} relies on hierarchical \code{par/or} and 
...
Consider the code that follows on the left side, which expands the sending 
trail of Figure~\ref{lst.ctp} (line 15):
...
The \code{buffer} packet is a local variable whose address is passed to native 
...
The code on the right above properly dequeues the packet if
...
The example of Figure~\ref{lst.orgs} introduces static organisms with three 
code chunks:
%
\begin{itemize}
...
%
Even tough the organisms run independently from the main program, they are 
still subject to the disciplined synchronous model, which keeps the whole 
system deterministic, as the static expansion of \emph{CODE-3} suggests (and 
based on the scheduler description of Section~\ref{sec.ceu.sync}).
%
...
% TODO: which first appeared in Sugar Cubes.
...
%- more functional (data + process => organism)
    %- less structured in this sense
