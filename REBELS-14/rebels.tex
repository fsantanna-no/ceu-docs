...
\section{Reactive Abstractions with Organisms}
\label{sec.ceu.orgs}
...
operation, just like the timeout pattern of Figure~\ref{lst.patts}.
...
primitive.
The example of Figure~\ref{lst.spawn} spawns a new instance of \code{Unit} on
every second and moves it to a random position.
A \code{spawn} returns a pointer to the new organism, which can be dereferenced 
through the operator \code{:} (the same as \code{->} in C/C++).
...
but differ on scoping and lifetime rules.
They also introduce pointers, which.
