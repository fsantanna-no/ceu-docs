...
structured programming, such as support for lasting loops and automatic 
...
TODO: present?
We promote the use of the language \CEU~\cite{ceu.sensys13}, a contemporary 
outlook of SRP that aims to expand it from the rigid embedded domain.
%
...
%
Our main contribution are the \emph{organisms}, a new abstraction mechanism to 
support parallel and \code{await} control structures and offer an object-like 
...
%
\begin{itemize}
\item Organisms are themselves alive and concurrent, acting as subprograms with 
continuous data and control state.
...
running trails%
\footnote{
The actual implementation enqueues incoming input events to process them in the 
incoming reactions.
}.
...
In the example of Figure~\ref{lst.intro}, accesses to variable \code{v} are 
always atomic because reactions to \code{1s} and \code{RESET} can never 
interleave.
%
...
Above all, the synchronous model empowers SRP with an \emph{orthogonal 
abortion} construct.
%
Orthogonal abortion is the ability to abort an activity
...
from outside it, without affecting the overall consistency of the system (e.g., 
properly releasing global resources).
%
...
%
The \code{par/or} is regarded as orthogonal because the composed activities do 
not know when and how they are aborted (i.e., abortion is external to them).
%
...
holding locks or pending messages).
%
The language runtime has to await the activity to be in a consistent state 
before aborting it, resulting in two unsatisfactory semantics for a 
\code{par/or} construct:
...
programmer (which assumes both activities have terminated), and also cause race 
conditions with the \code{par/or} continuation.
...
%In Section~\ref{sec.ceu.par} we show how abortion integrates safely with 
%activities that use stateful resources from the environment, such as file 
%handling and network transmissions.
...
In terms of control structures, the basic extension of SRP is the support for 
parallel compositions, allowing applications to handle multiple events 
concurrently.
%
...
The code in Figure~\ref{lst.ctp} describes part of a protocol for sensor 
networks ported to \CEU~\cite{wsn.ctp,ceu.sensys13}, which relies on 
\code{par/or} and \code{par/and} compositions to describe its state machine.
%
...
automatic variables and stateful $C$ functions (e.g., device drivers) to force 
proper finalization.

Consider the code on the left of Figure~\ref{lst.fin}, which sends the status 
packet of Figure~\ref{lst.ctp} (collapsed in line 15).
%
...
the radio driver.
%
\CEU refuses to compile programs like this and requests for \emph{finalization} 
