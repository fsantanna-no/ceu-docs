...
\begin{abstract}
Structured reactive programming (SRP) augments classical structured programming 
with continuous interaction with the environment.
%
We propose a new SRP abstraction mechanism, the \emph{organisms}, for the 
language \CEU.
%
Organisms extend objects with an execution body that composes multiple lines of 
execution to react to the environment independently.
%
...
environment.
%
SRP retains structured and sequential reasoning of concurrent programs and 
...
%
This is possible in synchronous languages due to the accurate control over the 
life cycle of concurrent activities, i.e., in between every reaction, the whole 
...
%
The same restriction applies to object-oriented programming, which can properly 
encapsulate data, but not control flow across reactions.
This way, although objects are the prevailing imperative abstraction mechanism 
for reactive applications (when used in conjunction with the observer pattern), 
they cannot provide proper SRP abstractions.
...
immediatelly to \code{dst=300} (line 20), and the second to move after 1 second 
...
memory.
%
This do not apply for static instances because the memory is preallocated 
within the scope and the organism identifier is accessible even if its body 
terminates.
...
state.
%
In contrast, FRP just uses declarative formulas to specify continuous functions 
over time, such as for physics or data constraints among entities, while SRP 
...
with a student that had no previous experience with the language in a 3-month 
project.
%
