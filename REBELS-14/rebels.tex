...
%~\cite{statecharts.reactive,rp.synchronous}.
...
libraries, such as Rx (from Microsoft), React (from Facebook), and 
Elm~\cite{frp.elm}.
%
...
reliance on side effects~\cite{rp.deprecating,rp.rescala}.
...
code chunks:
%
\emph{CODE-1} blinks two LEDs with different frequencies in parallel and 
terminates after 1 minute;
%
\emph{CODE-2} abstracts the blinking LEDs in an organism class and uses two 
instances of it to reproduce the same behavior of \emph{CODE-1};
%
\emph{CODE-3} is the semantically equivalent expansion of the organisms bodies, 
which resembles the original \emph{CODE-1}.
...
shared by all instances (just like objects share class methods).
...
\subsection{Pointers to organisms}
\label{sec.orgs.refs}
...
%
Independence of activities also makes distribution and many-core parallelism 
straightforward.
%
