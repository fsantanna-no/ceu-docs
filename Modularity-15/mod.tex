...
%
We propose a new SRP abstraction mechanism for the synchronous language \CEU:
%
...
environment.
%
SRP retains structured and sequential reasoning of concurrent programs which 
...
\code{v} in Figure~\ref{lst.intro} are atomic because reactions to events 
\code{1s} and \code{RESET} can never interrupt each other.
%
...
%pattern), they cannot provide proper SRP abstractions.
...
The core of the protocol first creates a vector of predefined clients (line 12) 
and pool to dynamically allocate forwarders (line 13).

The input event \code{RECEIVE} and \code{SENDONE} (line 2) handle pointers to 
radio packets in the radio driver:
\code{RECEIVE} notifies the program of an incoming packet in the node;
\code{SENDDONE} acknowledges a packet transmission first requested by the 
application through a \code{send_enqueue} call.

control the global state of the 
...
do
   loop seqno do
      ...
      with
         var _pkt_t pkt;
         _pkt_setRoute(&pkt, seqno);
         _pkt_setContents(&pkt, seqno);
         ...
         loop do
            ...
            var _pkt_t* done = await SENDDONE;
            if done == &pkt then
               break;
            end
         end
...
state vars
incr. scope
30% reduction in source code size
...
time, such as for physics or data constraints among entities, while SRP 
