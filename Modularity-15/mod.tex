...
The core of the protocol first creates a vector of clients and a pool to 
dynamically allocate forwarders (line 12-13).
Then, it enters a loop to continuously receive packets as they arrive and take 
the proper action (lines 15-25):
if the packet has no hops left, it reached the destination and the protocol 
calls the user-defined \code{\_receive} function to handle the packet (lines 
17-18);
