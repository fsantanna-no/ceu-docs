...
%The behavior of embedded systems is essentially reactive, interacting 
%permanently with the surrounding environment through I/O devices (e.g.  
%buttons, clocks, transceivers).
...
%, in particular, how to avoid \emph{glitches} and \emph{cyclic dependencies} 
%in dataflow programming~\cite{frp.survey}.
...
%working as a subroutine, which is expected to fill the variable \code{buf} 
%with the entry contents.
...
%the emerging area of Wireless Sensor Networks produced
...
%state machines~\cite{wsn.osm} (in comparison to \CEU's \code{await} 
%statement).
%
